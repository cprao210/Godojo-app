...
import { Mic, Speaker, Monitor, Keyboard, User, LogOut, Upload, ArrowUp, ArrowDown, ArrowLeft, ArrowRight } from 'lucide-react';
import { Camera, RotateCcw, Eye, Layout, MessageSquare, Crop, ChevronDown, Check, Power } from 'lucide-react';
import { Palette, Calendar, Ghost, Sun, Moon, RefreshCw, Info, Globe, FlaskConical, Terminal, Settings, Activity } from 'lucide-react';
import { ExternalLink, Trash2, Sparkles, Building2, MapPin } from 'lucide-react';
import { Zap, PointerOff, AlignLeft, Layers, Radio, Brain } from 'lucide-react';
import { Pause, StopCircle, GripVertical, BarChart2, Users } from 'lucide-react';
...
import { clampOverlayOpacity, OVERLAY_OPACITY_DEFAULT, OVERLAY_OPACITY_MIN, getDefaultOverlayOpacity } from '@/lib/overlayAppearance';
...
import { CustomSelectProps, ProviderSelectProps, SettingsOverlayProps } from '@/types';

// ---------------------------------------------------------------------------
// MockupDock — fake in-meeting widget for the opacity preview
// ---------------------------------------------------------------------------
const MockupDock = ({ opacity }: { opacity: number }) => {
...
const SettingsOverlay: React.FC<SettingsOverlayProps> = ({ isOpen, onClose, initialTab = 'general', deepLinkInviteToken = null, onDeepLinkTokenConsumed }) => {
    ...
    const handleAddTavilyKey = (e: React.ChangeEvent<HTMLInputElement>) => { setTavilyApiKey(e.target.value); setTavilyError(''); }
    ...
    useEffect(() => {
        if (isOpen) {
            ...
            if (window.electronAPI?.getZoomCalendarStatus) {
                                                            ...
                                                            <div className="flex items-start gap-4">
                                                                ...
                                                                <div>
                                                                    ...
                                                                    <p className="text-xs text-text-secondary mt-0.5">
                                                                        You are currently using GoDojo version {packageJson.version}
                                                                    </p>
                                                                </div>
                                                            </div>
                                                            <button
                                                                ...
                                                                <div>
                                                                    <div className="flex items-center gap-2 mb-1">
                                                                        ...
                                                                        {companyDossier.interview_difficulty && (
                                                                            <span className={`text-[9px] font-bold px-1.5 py-0.5 rounded border ${companyDossier.interview_difficulty === 'easy' ? 'bg-green-500/10 text-green-500 border-green-500/20' :
                                                                        ...
                                                                        {companyDossier.critics.map((c: any, i: number) => (
                                                                            <div key={i} className="bg-bg-input p-3 rounded-lg">
                                                                                <div className="flex items-center justify-between mb-1">
                                                                                    ...
                                                                                    <span className={`text-[9px] font-bold px-1.5 py-0.5 rounded border ${c.frequency === 'widespread' ? 'bg-red-500/10 text-red-400 border-red-500/20' :
