...
export const AudioStatusTray: React.FC<AudioStatusTrayProps> = ({ isVisible, onClose, onAllGranted }) => {
    ...
    return (
        <div className={`shrink-0 w-full h-12 border-t border-border-subtle ${isLight ? "bg-bg-secondary" : "bg-bg-elevated/40"} flex items-center px-4 z-40 relative`}>
                ...
                {createPortal(
                    ...
                    {(expanded) && (
                        <motion.div
                            ...
                            className={`fixed bottom-16 left-4 z-[400] w-[340px] ${isLight ? "bg-bg-secondary" : "bg-bg-card"} border border-border-subtle rounded-xl shadow-2xl overflow-hidden flex flex-col`}
                        >
                            {/* Header */}
                            <div className={`flex items-center justify-between px-4 py-3 border-b border-border-subtle ${isLight ? "bg-bg-elevated" : "bg-bg-component"}`}>
