...
function isMeaningfulTurn(turn: TranscriptTurn, cleanedText: string): boolean {
    // Always keep client speech (priority)
    if (turn.role === 'client' && cleanedText.length >= 5) {
        return true;
    }
 ...
 * Prioritizes client speech, keeps recent context
...
export function sparsifyTranscript(
    ...
    const otherTurns = turns.filter(t => t.role !== 'client');

    // Keep all client turns if under limit
    const result: TranscriptTurn[] = [];

    // Prioritize recent client turns (last 6)
    const recentClient = clientTurns.slice(-6);

    // Fill remaining with recent other turns
    const remainingSlots = maxTurns - recentClient.length;
    const recentOther = otherTurns.slice(-remainingSlots);

    // Merge and sort by timestamp
    result.push(...recentClient, ...recentOther);
...
export function formatTranscriptForLLM(turns: TranscriptTurn[]): string {
    return turns.map(turn => {
        const label = turn.role === 'client' ? 'CLIENT' :
